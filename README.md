...
A template of the sapper app with smelte/tailwind and typescript

## Steps to reproduce

- add file rollup.config.smelte.js. This contains a complete rollup bundeling process of the app, but is only used to create the css file "static/global_smelte.css".
- add dependency for the created css file to "src/template.html" head
```html
...
<link rel="stylesheet" href="global_smelte.css">
...
```
- add and update scripts in package.json: 
```javascript
{
    ...
  "scripts": {
    "dev": "npm run smelte-sapper && sapper dev",
    "build": "npm run smelte-sapper && sapper build --legacy",
    "export": "sapper export --legacy",
    "start": "node __sapper__/build",
    "validate": "svelte-check --ignore src/node_modules/@sapper",
    "smelte-sapper": "rollup -c rollup.config.smelte.js"
  },
...
}
...
For production build change "production" to true on rollup.config.smelte.js in root folder.
A bug like "[!] (plugin postcss) Error: ENOTDIR: not a directory, scandir 'src/components/Nav.svelte',"
might occur on the current smelte version, the fix is documented [here](https://github.com/matyunya/smelte/pull/185). To fix find and comment out the following line on node_modules/smelte/scr/utils/css-extractor.js
```javascript
function getComponentCodes(name) {
    const root = [
    "./src/components",//comment out this line!
